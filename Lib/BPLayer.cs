...
{
    [Serializable]
    class BPLayer<T> : IBackPropagation<T> where T: IComparable<T>
    {
        private int PreInputNum;
        private int OutputNum;

        private BPBase.PreInput[] PreInputLayer;
        private BPBase.Output<T>[] OutputLayer;

        private double learningRate = 0.2;

        public BPLayer(int preInputNum, int outputNum)
        {
            PreInputNum = preInputNum;
            OutputNum = outputNum;

            PreInputLayer = new BPBase.PreInput[PreInputNum];
            OutputLayer = new BPBase.Output<T>[OutputNum];
        }
