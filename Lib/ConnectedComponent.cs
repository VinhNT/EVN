...
namespace EVN.Lib
{
    class ConnectedComponent : IConnectedComponentLabeling
    {
        ...
        public IDictionary<int, ImageProperty> ProcessImage(Bitmap input)
        {
            _input = input;
            _width = input.Width;
            _height = input.Height;
            ...
            Dictionary<int, List<Pixel>> patterns = Find();
            var images = new Dictionary<int, ImageProperty>();

            foreach (KeyValuePair<int, List<Pixel>> pattern in patterns)
            {
                Bitmap bmp = CreateBitmap(pattern.Value);
                ImageProperty pro = new ImageProperty();
                pro.Heigth = bmp.Height;
                pro.Width = bmp.Width;
                pro.Position = FindMinLocation(pattern.Value);                                       
                images.Add(pattern.Key, pro);                
            }
        ...
        {
            return currentPixel.color.R < 70 && currentPixel.color.G < 70 && currentPixel.color.B < 70;
        }
        ...
        private Point FindMinLocation(List<Pixel> pattern)
        {
